...
int pte_clear_fpn(uint32_t *pte)
{
...
{                                                   // no guarantee all given pages are mapped
  uint32_t *pte = malloc(sizeof(uint32_t));         // page table entry
  ...
  {
    // The pte pointer is set to point to the page table entry for the current page number by adding pgn + pgit to
    //  the base address of the page table stored in caller->mm->pgd
...
int alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct **frm_lst)
{
  ...
  for (pgit = 0; pgit < req_pgnum; pgit++)
  {
    ...
    else
    {
      ...
      __swap_cp_page(caller->mram, PAGING_FPN_v2(caller->mm->pgd[vicpgn]), caller->active_mswp, swpfpn);
      pte_set_swap(&caller->mm->pgd[vicpgn], 1, swpfpn);
      pte_clear_fpn(&caller->mm->pgd[vicpgn]);
      MEMPHY_put_freefp(caller->mram, PAGING_FPN_v2(caller->mm->pgd[vicpgn]));
      pgit--;
    }
  }
  return 0;
...
int vm_map_ram(struct pcb_t *caller, int astart, int aend, int mapstart, int incpgnum, struct vm_rg_struct *ret_rg)
{
  ...
  int ret_vmap = vmap_page_range(caller, mapstart, incpgnum, frm_lst, ret_rg);
  if (ret_vmap < 0)
    return -1;
  return 0;
}
...
int print_pgtbl(struct pcb_t *caller, uint32_t start, uint32_t end)
{
  ...
  printf("\n");
  printf("print_pgtbl: start address: %d - end address: %d", start, end);
  if (caller == NULL)
  {
    printf("NULL caller\n");
    return -1;
