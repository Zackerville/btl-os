  ...
  for(pgit = 0; pgit < req_pgnum; pgit++)
  {
    if(MEMPHY_get_freefp(caller->mram, &fpn) == 0)
   {
      ...
      *frm_lst = newfp_str;
      ...
      {
        __swap_cp_page(caller->mram, victim_fpn , caller->active_mswp, swpfpn); // copy content on victim frane to swap menoгy space
      ...
      else
      {
        ...
        for (i = 0; i <  PAGING_MAX_MMSWP; i++)
        {
          ...
          {
            __swap_cp_page(caller->mram, victim_fpn, mswp + i, swpfpn); // copy content on victin frame to swap nemoгy space
            break;
          }
        }
      }
